...
void Node::OutputStudent() {
    ...
    for(; p; p=p->pNext) {
        ...
        out<<p->st.getname()<<" "<<p->st.getnumber()<<" "<<p->st.getage()<<" "<<p->st.getgender()<<" "<<p->st.getscore()<<"\n";
    }
    file.close();
}

void Node::ChangeStudent(QString name,long long number,QString age,QString gender,double score) {
    PNODE p = pHead->pNext;
    bool flag = false;
    for(; p; p = p->pNext) {
        if(number==p->st.getnumber()) {
...
bool Node::SearchStudent(QString &name,long long number,QString &age,QString &gender,double &score) {
    PNODE p = pHead->pNext;
    bool flag = false;
    for(; p; p = p->pNext) {
        if(number==p->st.getnumber()) {
